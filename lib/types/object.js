...
var ObjectIsaValidator = /** @class */ (function (_super) {
    ...
    Object.defineProperty(ObjectIsaValidator.prototype, "$type", {
        get: function () {
            var _this = this;
            return {
                $object: Object.keys(this.validatorMap).reduce(function (acc, key) {
                    acc[key] = _this.validatorMap[key].$type;
                    return acc;
                }, {})
            };
        },
    ...
    ObjectIsaValidator.prototype.field = function (options) {
        ...
        if (options.hasOwnProperty('optional') && options['optional'] === true) {
            return new ObjectIsaValidator(__assign(__assign({}, this.validatorMap), (_a = {}, _a[options.key] = validator.isOptional(), _a)), this);
        }
        else {
            return new ObjectIsaValidator(__assign(__assign({}, this.validatorMap), (_b = {}, _b[options.key] = validator, _b)), this);
        }
    };
    ObjectIsaValidator.prototype.delete = function (key) {
        var _this = this;
        var validatorMap = Object.keys(this.validatorMap).reduce(function (acc, propName) {
            if (propName !== key) {
                acc[propName] = _this.validatorMap[propName];
            }
            return acc;
        }, {});
    ...
    ObjectIsaValidator.prototype.subset = function (keys) {
        var _this = this;
        // keys should be 
        var keyMap = keys.reduce(function (acc, key) {
            acc[key] = key;
            return acc;
        }, {});
        var validatorMap = Object.keys(this.validatorMap).reduce(function (acc, propName) {
...
var ObjectMapIsaValidator = /** @class */ (function (_super) {
    ...
    Object.defineProperty(ObjectMapIsaValidator.prototype, "$type", {
        get: function () { return { $objectMap: (_isa_1.isFunction(this.inner) ? this.inner() : this.inner).$type }; },
        enumerable: false,
        configurable: true
    });
