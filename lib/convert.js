...
const { isConvertValidator , isConvertValidatorCompat , isArrayOf , isConstraint } = require('./_isa');

function BaseConvertValidator (convertOptions) {
    if (!convertOptions) {
        // console.log(`*************** ${this.constructor.name}.ctor:ERROR:NO_CONVERT_OPTIONS`, convertOptions)
...
function OptionalConvertValidator(validator, convertOptions) {
    // console.log(`************** ${this.constructor.name}.ctor`, validator, convertOptions)
    BaseConvertValidator.call(this, convertOptions);
    this.validator = validator;
}
...
UnionConvertValidator.prototype.validate = function validate(value, path = '$') {
    return this.validators.reduce((result, validator, i) => {
        return result.cata((v) => {
            ...
            // console.log(`******** UnionConvertValidator.validate:ERROR`, value, i, e.name);
            return (isConvertValidator(validator) ? validator : validator()).validate(value, path)
        });
    }, reject({
        error: 'UnionIsaError',
        path,
